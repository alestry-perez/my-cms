...
$(function () {
    ...
    function getCounterRowHtml(counter) {
        return `
        <div class="row" data-counter-id="${counter.id}">
    ...
    function installCounterClickHandler() {
        ...
        function counterClickHandler(event) {
            ...
            $.post(apiRoot + "/" + counterId, {}, function (counter, status) {
                $(".row[data-counter-id=" + counterId + "]").replaceWith(
                    getCounterRowHtml(counter)
                );
